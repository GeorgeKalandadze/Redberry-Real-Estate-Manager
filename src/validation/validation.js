...
// export const validateMinMax = (values) => {
//   const response = {
//     price: {
//       from: "",
//       to: "",
...
//     !values?.price?.from ||
//     !values?.price?.to ||
//     isNaN(values.price.from) ||
//     isNaN(values.price.to)
//   ) {
//     response.price = { from: "invalid", to: "invalid" };
//   } else if (Number(values.price.from) > Number(values.price.to)) {
//     response.price = { from: "invalid", to: "invalid" };
//   } else {
//     response.price = { from: "valid", to: "valid" };
//   }

//   if (
//     !values?.area?.from ||
//     !values?.area?.to ||
//     isNaN(values.area.from) ||
//     isNaN(values.area.to)
//   ) {
//     response.area = { from: "invalid", to: "invalid" };
//   } else if (Number(values.area.from) > Number(values.area.to)) {
//     response.area = { from: "invalid", to: "invalid" };
//   } else {
//     response.area = { from: "valid", to: "valid" };
//   }
...
export const validateMinMax = (values) => {
  const validateRange = (from, to) => {
    if (!from || !to || isNaN(from) || isNaN(to) || Number(from) > Number(to)) {
  ...
  return {
    price: validateRange(values?.price?.from, values?.price?.to),
    area: validateRange(values?.area?.from, values?.area?.to),
  };
};
